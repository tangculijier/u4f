...
public class MapDistance
{
			...
			String lat2Str, String lng2Str)
	{
		...
		if (distance > 1)
		{
			distance = take_effect_number(distance, 4);
			return distance + "km";
		} else
			//小于1km, 保留1位小数
		{
			distance = take_effect_number(distance * 1000, 1);
			return distance + "m";
		}
	...
	// double保留几位小数，位数代表小数点后保留几位
	public static double take_effect_number(double number, int weishu)
	{
		BigDecimal temp = new BigDecimal(number);
		return number = temp.setScale(weishu, BigDecimal.ROUND_HALF_UP)
				.doubleValue();
	}
